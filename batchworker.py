...
# -------- main worker --------------------------------------------------------
async def batch_worker(model, batch_ms: float = 15.0, max_batch: int = 32):
    ...
    while True:
        ...
        # ---------- inference ----------
        try:
            logger.info("Running recognize on %s", batch)
            # We run the transcription in an asyncio thread to avoid blocking the event loop
            def _run_batch_recognize(paths):
                return [model.recognize(p) for p in paths]
            outs = await asyncio.to_thread(_run_batch_recognize, batch)
            logger.info("Recognize finished!")
        ...
        for p, h in zip(batch, outs):
            # In ONNX ASR the output might be text or an object with text property depending on config
